#!/usr/bin/env bash
#
# deploy-hetzner.sh — one-command deploy of the Open Managed Agents to a
...
#     HCLOUD_SERVER_NAME=open-managed-agents   # run multiple deploys by setting different names
...
SERVER_NAME="${HCLOUD_SERVER_NAME:-open-managed-agents}"
SSH_KEY_NAME="${HCLOUD_SERVER_NAME:-open-managed-agents}-key"
