#!/usr/bin/env bash
#
# deploy-gcp-compute.sh — one-command deploy of the Open Managed Agents
...
#     GCE_INSTANCE_NAME=open-managed-agents        # run multiple deploys by setting different names
...
#     OPENCLAW_DEPLOY_REPO=https://github.com/stainlu/open-managed-agents.git
#
...
INSTANCE_NAME="${GCE_INSTANCE_NAME:-open-managed-agents}"
...
REPO_URL="${OPENCLAW_DEPLOY_REPO:-https://github.com/stainlu/open-managed-agents.git}"
...
if gce firewall-rules describe "${FIREWALL_RULE_ORCH}" >/dev/null 2>&1; then
    ...
    gce firewall-rules create "${FIREWALL_RULE_ORCH}" \
        ...
        --description 'Allow public access to Open Managed Agents orchestrator HTTP API' \
        --quiet >/dev/null
    printf "    %-18s created.\n" "firewall rule:"
fi
