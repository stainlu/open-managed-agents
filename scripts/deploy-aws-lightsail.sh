#!/usr/bin/env bash
#
# deploy-aws-lightsail.sh — one-command deploy of the Open Managed Agents
...
#     LIGHTSAIL_INSTANCE_NAME=open-managed-agents    # run multiple deploys by setting different names
...
INSTANCE_NAME="${LIGHTSAIL_INSTANCE_NAME:-open-managed-agents}"
...
if ls_cli get-instance --instance-name "${INSTANCE_NAME}" >/dev/null 2>&1; then
    ...
    ls_cli create-instances \
        ...
        --bundle-id "${BUNDLE_ID}" \
        --user-data "${USER_DATA}" \
        --tags 'key=managed-by,value=open-managed-agents' >/dev/null
