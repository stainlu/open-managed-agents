...
<title>Open Managed Agents — Console</title>
<style>
  :root {
    --bg: #0d1117;
    --bg-elev: #161b22;
...
<header>
  ...
  <a href="https://github.com/stainlu/open-managed-agents" target="_blank">github</a>
</header>
...
function renderErrorBanner(rawError) {
  ...
  if (/mcp_oauth credential .* refresh failed/i.test(raw)) {
    ...
    body = "The provider rejected the request — usually a bad / expired API key in the orchestrator's env, or a rate-limit at the provider. Check <code>docker logs open-managed-agents-orchestrator</code> and the provider's dashboard.";
    autoShowRaw = false;
