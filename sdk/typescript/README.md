# Open Managed Agents TypeScript SDK

TypeScript/JavaScript client for the [Open Managed Agents](https://github.com/stainlu/open-managed-agents) API. Zero runtime dependencies — uses native `fetch`.

## Install

```bash
npm install @stainlu/open-managed-agents
```
...
```ts
import { OpenClawClient } from "@stainlu/open-managed-agents";
...
```ts
import { OpenClawClient, OpenClawError } from "@stainlu/open-managed-agents";
