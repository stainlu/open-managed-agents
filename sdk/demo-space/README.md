---
title: Open Managed Agents Demo
emoji: 🦀
colorFrom: indigo
colorTo: pink
sdk: gradio
...
pinned: false
license: mit
short_description: Live chat demo for Open Managed Agents.
tags:
  - agents
  - orchestration
  - openclaw
...
# Open Managed Agents — HuggingFace Space demo

A Gradio chat UI that talks to a real [Open Managed Agents](https://github.com/stainlu/open-managed-agents) orchestrator. The Space itself only runs the UI; every message opens or reuses a durable session on the orchestrator you point it at.
...
```bash
git clone https://github.com/stainlu/open-managed-agents
...
hf repo create open-managed-agents-demo --repo-type space --space-sdk gradio
```
...
```bash
hf upload <your-username>/open-managed-agents-demo sdk/demo-space . --repo-type space
```

Every push rebuilds the Space automatically. Within ~1 minute the app is live at `https://huggingface.co/spaces/<your-username>/open-managed-agents-demo`.
