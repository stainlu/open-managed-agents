...
const spawnOptions: SpawnOptions = {
  ...
  labels: { "session-id": sessionId, "agent-id": agent.agentId, "managed-by": "open-managed-agents" },
};
```
...
**The refactor proposed in sections 5–7 was rejected. We will NOT build `SessionFileStore`, `WorkspaceProvisioner`, a Cloud Run adapter, or a Fargate adapter as core backends.** Instead, Open Managed Agents targets **multi-provider cheap VPSes** as its cloud story, reusing the existing `DockerContainerRuntime` unchanged.
