...
Run the Open Managed Agents on a Google Cloud Compute Engine instance for **$0-$25/month** — GCE's fixed-price VM family, the closest GCP match to AWS Lightsail and Hetzner Cloud. One command, about 4-5 minutes end-to-end from zero.
...
#   GCE_INSTANCE_NAME=open-managed-agents   # default; change to run multiple deploys
...
==> Provisioning e2-medium instance in us-central1-a (open-managed-agents)
    IPv4:              34.123.45.67
==> Opening port 8080 (firewall rule: open-managed-agents-allow-orchestrator)
...
==> Deploy complete
    ...
    SSH:               gcloud compute ssh ubuntu@open-managed-agents --zone us-central1-a
                       # or directly: ssh ubuntu@34.123.45.67
    Tail bootstrap:    gcloud compute ssh ubuntu@open-managed-agents --zone us-central1-a --command 'sudo tail -f /var/log/openclaw-bootstrap.log'
```
...
```bash
ssh ubuntu@<ip> "sudo bash -lc 'cd /opt/openclaw && git pull && docker compose pull && docker pull ghcr.io/stainlu/open-managed-agents-egress-proxy:latest && docker compose up -d'"
```
...
# Deletes the instance + the open-managed-agents-allow-orchestrator firewall rule.
```

Or manually:

```bash
gcloud compute instances delete open-managed-agents --zone us-central1-a --quiet
gcloud compute firewall-rules delete open-managed-agents-allow-orchestrator --quiet
```
...
git clone --depth 1 https://github.com/stainlu/open-managed-agents.git /opt/openclaw
cd /opt/openclaw
mkdir -p data/sessions data/state
echo "MOONSHOT_API_KEY=$MOONSHOT_API_KEY" > .env
docker compose pull
docker pull ghcr.io/stainlu/open-managed-agents-egress-proxy:latest
docker compose up -d
SCRIPT
...
gcloud compute instances create open-managed-agents \
  --zone us-central1-a \
  --machine-type e2-medium \
  ...
  --tags open-managed-agents \
  --metadata-from-file startup-script=/tmp/startup.sh,ssh-keys=/tmp/ssh-keys.txt \
  --labels managed-by=open-managed-agents

# 5. Open port 8080 scoped to the instance tag
gcloud compute firewall-rules create open-managed-agents-allow-orchestrator \
  --allow tcp:8080 \
  --source-ranges 0.0.0.0/0 \
  --target-tags open-managed-agents

# 6. Get the public IP
IP=$(gcloud compute instances describe open-managed-agents \
  --zone us-central1-a \
...
- **Instance reaches "running" but `/healthz` never responds**: startup-script is still running or crashed. SSH in with `gcloud compute ssh ubuntu@<instance>` and inspect `sudo tail -f /var/log/openclaw-bootstrap.log`. Also check the serial console: `gcloud compute instances get-serial-port-output open-managed-agents --zone us-central1-a`.
...
- **`/healthz` never returns OK but docker compose started successfully**: port 8080 is likely firewalled. Check `gcloud compute firewall-rules list --filter="name=open-managed-agents-allow-orchestrator"` and verify the target tag matches the instance tag.
