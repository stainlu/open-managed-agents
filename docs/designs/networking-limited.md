...
That honesty was correct at the time. Claude Managed Agents ships network-allowlist enforcement as a first-class capability, and Open Managed Agents needed to ship the same enforcement story to credibly claim "the open alternative to Claude Managed Agents."
...
- `ghcr.io/stainlu/open-managed-agents-egress-proxy:latest` published to GHCR.
