...
`wrangler.toml` bindings, live Workflow deployment, and real deployment
verification remain open. A `ManagedRunScheduler` boundary, Cloudflare Workflow
scheduling adapter, token-gated coordinator re-entry route, and Workflow runner
helper now exist, so the remaining Cloudflare gap is deployment wiring and
runtime proof rather than deciding where background run kickoff belongs.
...
boundary, and a D1-compatible managed event backend now exists without
requiring a local `stateRoot`. Harnesses also declare whether they need a
container runtime endpoint or can run natively, so the router can invoke
