...
Run the Open Managed Agents on an **AWS Lightsail** instance for **$5–$24/month** — AWS's own "cheap VPS with Docker" product, priced as a fixed monthly bundle and designed for exactly the workloads that Item 10a proved on Hetzner. One command, about 6 minutes end-to-end from zero.
...
#   LIGHTSAIL_INSTANCE_NAME=open-managed-agents  # default; change to run multiple deploys
...
==> Provisioning medium_3_0 instance in us-east-1a (open-managed-agents)
...
**The deploy uses pre-built images from GHCR** — `ghcr.io/stainlu/open-managed-agents-{orchestrator,openclaw-agent}:latest`, published by `.github/workflows/publish-images.yaml` on every push to `main`. Deploy time dropped from ~12 min (earlier build-from-source baseline) to ~5 min because `npm install -g openclaw@2026.4.26` (6.5 min of the original 12) no longer runs on the target VM. CPU burst credits also stay full for the first turn, shortening cold-spawn time as a side effect.
...
# or: aws lightsail delete-instance --instance-name open-managed-agents
```
...
runcmd:
  ...
  - git clone https://github.com/stainlu/open-managed-agents.git /opt/openclaw
  - cd /opt/openclaw && echo "MOONSHOT_API_KEY=$MOONSHOT_API_KEY" > .env && docker compose pull && docker pull ghcr.io/stainlu/open-managed-agents-egress-proxy:latest && docker compose up -d
CLOUDINIT
)"

# 2. Create the instance
aws lightsail create-instances \
  --instance-names open-managed-agents \
  --availability-zone us-east-1a \
  --blueprint-id ubuntu_24_04 \
  --bundle-id medium_3_0 \
  --user-data "$USER_DATA" \
  --tags 'key=managed-by,value=open-managed-agents'

# 3. Wait for it to reach 'running' state
while [ "$(aws lightsail get-instance --instance-name open-managed-agents --query 'instance.state.name' --output text)" != "running" ]; do
  sleep 5
done

# 4. Open port 8080 (and keep 22 open for SSH)
aws lightsail put-instance-public-ports \
  --instance-name open-managed-agents \
  --port-infos 'fromPort=22,toPort=22,protocol=tcp' 'fromPort=8080,toPort=8080,protocol=tcp'

# 5. Get the public IP
IP=$(aws lightsail get-instance --instance-name open-managed-agents --query 'instance.publicIpAddress' --output text)
