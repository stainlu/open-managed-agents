...
Run a full Open Managed Agents on a **€3.99/month** Hetzner CAX11 (ARM Ampere) — the cheapest credible production backend for the runtime. One command, about 6 minutes end-to-end from zero.
...
**ARM compatibility**: Open Managed Agents ships multi-arch Docker images. Node 22, better-sqlite3, OpenClaw core, and Pi all build cleanly on ARM64 Linux. If you prefer Intel x86, override `HCLOUD_SERVER_TYPE=cx23` (€4.99/mo net) — same specs, same image build path, slightly more expensive.
...
#   HCLOUD_SERVER_NAME=open-managed-agents    # default; change to run multiple deploys
...
==> Registering SSH key with Hetzner project (open-managed-agents-key)
==> Rendering cloud-init user-data with MOONSHOT_API_KEY
==> Provisioning cax11 server in nbg1 (open-managed-agents)
    IPv4:              5.75.123.45
...
==> Deploy complete
    ...
    Destroy with:      hcloud server delete open-managed-agents
```
...
```bash
ssh root@<ip> 'cd /opt/openclaw && git pull && docker compose pull && docker pull ghcr.io/stainlu/open-managed-agents-egress-proxy:latest && docker compose up -d'
```
...
```bash
hcloud server delete open-managed-agents
...
hcloud server create \
  ...
  --image ubuntu-24.04 \
  --location nbg1 \
  --ssh-key open-managed-agents-key \
  --user-data-from-file ./scripts/hetzner-cloud-init.yaml

# 3. Grab the IP
IP=$(hcloud server describe open-managed-agents -o json | jq -r .public_net.ipv4.ip)
