...
async function main() {
  ...
  try {
    ...
    if (promotion) assertCloudflareRuntimeReady(health.runtime);
    recordCheck("health", {
      version: health.version ?? "unknown",
      runtime: runtimeEvidence(health.runtime),
    });
    console.log(
      `ok health version=${health.version ?? "unknown"} runtime=${health.runtime?.stack ?? "unknown"}`,
    );
    ...
    if (promotion) assertCloudflareRuntimeReady(runtimeProfile.runtime, "runtime profile");
    recordCheck("runtime_profile", {
      runtime: runtimeEvidence(runtimeProfile.runtime),
    });
    console.log(
      `ok runtime profile platform=${runtimeProfile.runtime.platform ?? "unknown"} stack=${runtimeProfile.runtime.stack ?? "unknown"}`,
    );
