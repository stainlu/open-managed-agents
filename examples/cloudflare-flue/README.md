...
- The current Flue adapter owns direct prompt, task, and shell paths for
  ...
  built-in Flue tool policy parity beyond `bash` are intentionally not faked.
  Exact deny and `always_ask` approval filtering are limited to URL-MCP tools
  that OMA connects into Flue and Flue's built-in `bash` tool at OMA's managed
  `SessionEnv.exec()` boundary.
